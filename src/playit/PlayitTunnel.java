...
public final class PlayitTunnel
{
	 ...
	 */
	private static final AgentVersion AGENT_VERSION = new AgentVersion( "f4e73f52-f35c-4f18-9ab2-3aaa5c4488c1", 0, 16, 0 );
	private static final String AGENT_VERSION_STRING = "0.16.0";

	public enum Phase
	{
		OFF, CONNECTING, ONLINE, ERROR, INVALID_AUTH
	}
	...
	public PlayitTunnel( String secretKey, int minecraftPort, Consumer<String> activity )
	{
		...
		this.activity = activity == null ? message ->
		{
		} : activity;
	}
			...
			try
			{
				...
				if( setupResult instanceof ApiSuccess<ClaimSetupResponse, ?> success )
				{
					switch( success.data() )
					{
						...
						default ->
						{
							/* WaitingForUserVisit / WaitingForUser: seguir sondeando */ }
					}
				}
			}
	...
	private static String findMinecraftTunnelAddress( ApiClient api ) throws ApiClientException
	{
		...
		if( !(tunnelsResult instanceof ApiSuccessNoFail<AccountTunnelsV1> success) )
			return null;
		if( success.data().tunnels() == null )
			return null;
	...
	private static String displayAddress( AccountTunnelV1 tunnel )
	{
		...
		if( first instanceof ConnectAddress.Ip4 ip4 )
			return ip4.value().address() + ":" + ip4.value().default_port();
		if( first instanceof ConnectAddress.Ip6 ip6 )
			return ip6.value().address() + ":" + ip6.value().default_port();
		if( first instanceof ConnectAddress.Addr4 addr4 )
			return addr4.value().address();
		if( first instanceof ConnectAddress.Addr6 addr6 )
			return addr6.value().address();
		if( first instanceof ConnectAddress.Auto auto )
			return auto.value().address();
		if( first instanceof ConnectAddress.Domain domain )
			return domain.value().address();
		return null;
	}
