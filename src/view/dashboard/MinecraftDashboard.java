...
public final class MinecraftDashboard extends JPanel
{
	...
	public enum Phase
	{
		NO_SERVER("NO SERVER", TEXT_MUTED), DISCOVERING("DISCOVERING", AMBER), OFFLINE("OFFLINE", TEXT_MUTED), REMOTE_HOST("REMOTE HOST",
				CYAN), SYNCING("SYNCING", AMBER), IMPORTING("IMPORTING", AMBER), STARTING("STARTING",
						AMBER), ONLINE("ONLINE", GREEN), STOPPING("STOPPING", AMBER), SAVING("SAVING", AMBER), ERROR("ERROR", RED);
	...
	public JTextField settingsNetworkInput()
	{
		return settingsNetworkInput;
	}
	public JTextField settingsPortInput()
	{
		return settingsPortInput;
	}
	public JTextField settingsRamInput()
	{
		return settingsRamInput;
	}
	public JTextField settingsMaxPlayersInput()
	{
		return settingsMaxPlayersInput;
	}
	...
	private JPanel buildOnboardingPage()
	{
		...
		primaryCopy.add(
				DashboardTheme.label( "Create a new Forge world or open one already installed on this machine.", TEXT, 14, Font.PLAIN ) );
		primaryCopy.add( Box.createVerticalStrut( 7 ) );
		primaryCopy.add( DashboardTheme.label(
				"The dashboard only displays host, port, RAM and backup data after a valid startup folder is selected.", TEXT_MUTED, 11,
				Font.PLAIN ) );
		...
		invitedCopy.add( DashboardTheme.label(
				"Already invited by another host? Accept the invitation and clone the private server repository.", TEXT, 13, Font.PLAIN ) );
		invitedCopy.add( Box.createVerticalStrut( 6 ) );
		invitedCopy.add( DashboardTheme.label( "The clone receives world data and mods; port, RAM and machine settings remain local.",
	...
	private JPanel buildBackupsPage()
	{
		...
		for( JButton button : List.of( signInButton, profileButton, createRepositoryButton, syncButton ) )
		{
			DashboardTheme.styleButton( button,
					button == createRepositoryButton ? DashboardTheme.ButtonKind.PRIMARY : DashboardTheme.ButtonKind.SECONDARY );
		...
		collaborationCopy.add(
				DashboardTheme.label( "Invited hosts pull before start and push only after a clean stop.", TEXT_MUTED, 11, Font.PLAIN ) );
	...
	{
		JPanel page = pagePanel();
		page.add( sectionHeading( "LOCAL SERVER SETTINGS",
				"Edit the selected server here. Changes are locked while any host is using the world." ) );
		...
		note.add( DashboardTheme.label( "server.properties and user_jvm_args.txt are protected as local-only files.", TEXT, 12,
				Font.PLAIN ) );
		note.add( Box.createVerticalStrut( 5 ) );
		note.add( DashboardTheme.label( "World data, mods and shared server assets remain synchronized through the selected provider.",
				TEXT_MUTED, 11, Font.PLAIN ) );
	...
	private void updateMetrics()
	{
		...
		updateMetric( "host", state.hostAddress(), state.phase() == Phase.REMOTE_HOST ? "remote peer" : "discovered address",
				state.phase() == Phase.REMOTE_HOST ? CYAN : TEXT );
		...
		updateMetric( "repository", state.repositoryLinked() ? "LINKED" : "NOT LINKED", state.repository(),
				state.repositoryLinked() ? GREEN : TEXT_MUTED );
		updateMetric( "sync", state.syncState(), "world protection", syncColor() );
		updateMetric( "lastSync", state.lastSync(), "confirmed push", TEXT );

		updateMetric( "networkState", state.phase() == Phase.DISCOVERING ? "SCANNING" : "READY", state.networkName(),
				state.phase() == Phase.DISCOVERING ? AMBER : GREEN );
		updateMetric( "networkHost", state.hostAddress(), state.phase() == Phase.REMOTE_HOST ? "remote peer" : "local / none",
				state.phase() == Phase.REMOTE_HOST ? CYAN : TEXT );
		updateMetric( "networkPort", state.port(), "Minecraft Java", TEXT );
		updateMetric( "networkRole", state.phase() == Phase.ONLINE ? "HOST" : "CLIENT", "host election",
				state.phase() == Phase.ONLINE ? GREEN : TEXT );
	}
	...
	private void updateServers()
	{
		...
		if( incoming.equals( renderedServerEntries ) )
			return;
	...
	private void updateActionAvailability()
	{
		...
		for( JButton importButton : importWorldButtons )
			importButton.setEnabled( canImport );
		for( JButton pullButton : pullWorldButtons )
	...
	private void updateMetric( String key, String value, String detail, Color valueColor )
	{
		...
		if( card != null )
			card.setValue( value, detail, valueColor );
	}
	...
	private JPanel serverRow( ServerEntry entry )
	{
		...
		if( entry.selected() )
			row.setBackground( ACTIVE_BACKGROUND );
		...
		{
			// La direccion a la vista en el propio tablero: lo que se pega en Minecraft
			copy.add( Box.createVerticalStrut( 2 ) );
			copy.add( DashboardTheme.label( entry.connectAddress(), CYAN, 10, Font.PLAIN ) );
		}
		row.add( copy, BorderLayout.CENTER );
